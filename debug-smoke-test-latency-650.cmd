...
REM Live latency smoke test (voice_stop_ms=650)
REM Usage:
REM   .\debug-smoke-test-latency-650.cmd [mic_selector]
...
REM Notes:
REM - Writes profile output to: debug-smoke-test-latency-650.prof.txt
REM - Uses tools\run-live-ab.ps1 (Release by default) and enables --profile

set "ROOT=%~dp0"
pushd "%ROOT%" >nul

REM Prevent recursive window spawning
...
set "OUT=debug-smoke-test-latency-650.prof.txt"

echo Running LIVE latency smoke (stop=650ms)
echo   MIC: %MIC%
echo   OUT: %OUT%
echo(

REM Show output live in terminal for interactive feedback
powershell -NoProfile -ExecutionPolicy Bypass -File "%ROOT%tools\run-live-ab.ps1" -Mic "%MIC%" -OmitFeatures streamerbot,lang-fallback-fr --voice-stop-ms 650 --profile
