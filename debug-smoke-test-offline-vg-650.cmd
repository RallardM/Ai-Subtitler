...
start "Ai-Subtitler Smoke (Offline VG 650)" cmd.exe /k ""%~f0" %*"
exit /b 0
:after_spawn
...
REM   debug-smoke-test-offline-vg-650.cmd C:\path\to\file.wav
set "WAV=%~1"
...
set "OUT=debug-smoke-test-offline-vg-650.vg.txt"
set "ERR=debug-smoke-test-offline-vg-650.err.txt"

echo Running OFFLINE voice-gate test (stop=650ms)
echo   WAV: %WAV%
echo   OUT: %OUT%
echo   ERR: %ERR%
echo(

cmd.exe /c ""%~dp0run.cmd" --test-voice-gate "%WAV%" --vad-check-ms 50 --voice-stop-ms 650 1>"%OUT%" 2>"%ERR%""
